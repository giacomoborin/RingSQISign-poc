from setup import p, O0, logp, loglogp
from KLPT import derive_L, RepresentIntegerHeuristic
from ideals import left_isomorphism, pullback_ideal

def distinguisher(JJ, Iτ, debug = False):
    """
    A simple and known attack to distinguish simulated transcripts
    from original ones, given the secret key Itau
    """
    # JJ is an O1 ideal of norm 2^*
    # Iτ is the secret ideal

    # preparation
    O0 = Iτ.left_order()
    O1 = Iτ.right_order()
    Nτ = Iτ.norm()

    assert O1 == JJ.left_order(), 'input ideal defined on the wrong order'

    prime_ideal_try = 10
    gamma_try = 1000
    for _ in range(prime_ideal_try):
        # derivation of the prime norm ideal, expected <5 trial
        L,NL,isom_LN = derive_L(JJ, Iτ, Nτ, O0, O1)

        # recalculation of e1 from N
        e1 = floor(logp - log(NL, l) + 1.74 * loglogp) 
        L1 = l**e1

        # recalculation of possible betas, we may need the scaling
        # scaling = 2 # in [1..32]
        # beta = (left_isomorphism(L,pullback_ideal(O0, O1, JJ.scale(scaling), Iτ)) * NL).conjugate() 
        beta = (left_isomorphism(L,pullback_ideal(O0, O1, JJ, Iτ)) * NL).conjugate() 
        repr_int = NL * L1
        for _ in range(gamma_try):
            gamma = RepresentIntegerHeuristic(repr_int)
            mu = gamma**-1 * beta
            # the factor 32*9 is beacuse of the possible scaling of the ideal happened in KLPT 
            if 32*9*5*mu in O0:
                if 0 == mu[0] % (Nτ*NL) and 0 == mu[1] % (Nτ*NL):
                    print('Generated by KLPT')
                    return NL,gamma

    print('Not generated by KLPT')
    return 0
